...
- `sessions.topBar` — when true (`assist config set sessions.topBar true -g`), the sessions view moves the active session's phase description, elapsed time and restored indicator off its card into a sticky top bar inside the terminal panel (served to the UI by `GET /api/session-layout`). The bar stays in the terminal half when the PR preview split opens and is absent in the transcript view; cards keep their chips, title, status dot, undurable reason, context % and change counts. Defaults to false, which renders the view exactly as before.
