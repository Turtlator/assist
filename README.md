...
Concurrent sessions in one repo can be isolated with native git worktrees instead of keeping multiple physical clones. Opt in per repo (default off): `assist config set worktree.enabled true -g --repo`. A lone session runs in the clone's own tree unchanged; a concurrent session spills into an adjacent `<clone>-N` worktree with its own tree, index, and HEAD. New worktrees are seeded so they build and run — a per-worktree dependency install (`worktree.install`: `true` auto-detects the package manager from the lockfile, or give an explicit command; `false` skips it — `node_modules` is never shared) and a copy of the configured gitignored files (`worktree.copy`, default `.env`, `settings.local.json`, `.claude/settings.local.json`). Worktrees hang beside the clone under `worktree.root` (default: the clone's parent). A worktree is reaped only when its session reaches `done`, and a session cannot reach `done` while its tree is dirty or holds local-only commits (`push:false`) — the card shows why (`waiting — unpushed commits`) and the daemon fs-watches the tree's git state, auto-reaping the moment the branch becomes durable (pushed). Worktrees orphaned across a daemon restart are reconciled on startup against a persisted registry (`~/.assist/worktrees.json`); nothing is removed while undurable. With worktree mode off, behaviour is unchanged.
